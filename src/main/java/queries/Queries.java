...
public class Queries {
    ...
    public static void createTable(String tableName, String query) {
        ...
        String activeDatabasePath = "databases/" + state.getActiveDatabase();
        List<String> tableList = fileReadWrite.getDirectories(activeDatabasePath);

        String[] tableNameParts = tableName.split("\\(");
        String trimmedTableName = tableNameParts[0].trim();

        if (tableList.contains(trimmedTableName.toUpperCase())) {
            System.out.println("Table already exists. Please choose a different name.");
            return;
        }

        StringBuilder metaContent = new StringBuilder();
        metaContent.append("TABLE^").append(trimmedTableName).append("\n");
        ...
        if (matcher.find()) {
            ...
            String tablePath = activeDatabasePath + "/" + trimmedTableName.toUpperCase();
            fileReadWrite.createDirectory(tablePath);
            fileReadWrite.writeFile(tablePath + "/METADATA", metaContent.toString());
            fileReadWrite.writeFile(activeDatabasePath + "/METADATA", metaContent.toString());
    ...
    /*public static void insertIntoTable(String tableName, String values) {
        ...
        String tablePath = "databases/" + "/" + tableName.toUpperCase();
        ...
        if (matcher.find()) {
            String[] valueCommands = matcher.group(1).split(",");
            for (String command : valueCommands) {
                command = command.trim();

                if (command.startsWith("'") && command.endsWith("'")) {
                    valueList.add(command.substring(1, command.length() - 1));
                } else {
                    valueList.add(command);
                }
            }
        } else {
            System.out.println("Invalid syntax. Please use the following syntax: INSERT INTO <tableName> VALUES (value1, value2, ...)");
            return;
        }

        // Get the column names and types from the metadata file
        List<String[]> columns = new ArrayList<>();
        String metaPath = tablePath + "/METADATA";
        String l = fileReadWrite.readFile(metaPath);
        ...
        for (String line : lines) {
            ...
            if (parts.length == 3 && parts[0].equals("COLUMN")) {
                columns.add(new String[]{parts[1], parts[2]});
            }
        }
        ...
        String data = rowData.toString().substring(0, rowData.length() - 1) + "\n";
        fileReadWrite.writeFile(tablePath + "/DATA", data);
